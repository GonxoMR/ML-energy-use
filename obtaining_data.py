...
apiDic = pd.read_csv('apiKeyDictionary.csv')
types = np.unique(apiDic['type'])

# Creating a saving file
store = pd.HDFStore(dir+str(datetime.datetime.now())+'_feeds.h5')
	...
	for index, row in apiDic.loc[(apiDic['type']==type),['key','id']].iterrows():
		...
		if len(response.content) % 4 == 0:
			...
			if array[-1] != float(meta['value']):
				print("Error: The last value of the array do not match the last value obtained from the feed.")
			...
			print("Error: The number of elements recieved is not multiple of 4. Decodification is not posible")
